# scripts/analyze_ga_stats.py
"""
Statistical analysis for Genetic Algorithm (GA) batch results.

Core kept (same as HC, adapted for GA):
...
- Original stability tests (concise bullets): Welch’s t, MW p, Cohen’s d, rank-biserial r
- Descriptives (overall + by tag) with compact, human-friendly tables
- Feasibility rate + histogram (integer-aligned bins; no outlines)
- Spearman heatmap + 3 key scatterplots (legend + stats box stacked in upper-right)
- Top-5 tables
- Tee console to Markdown: results/summary/ga_analysis_output.md
- Save & show plots (Spyder Plots pane)
- Enhanced results saved BOTH as UTF-8-BOM CSV (Excel-safe) and ASCII-header XLSX 

Optional:
- GA param correlations (skips constants)
- GA ops vs metrics correlations (cx_ops, mut_ops, repair_calls, repair_per_op)
"""
...
RUNS_DIR = ROOT / "results" / "Genetic_algorithm_runs"
CSV_PATH = RUNS_DIR / "ga_summary_log.csv"
PLOTS_DIR = ROOT / "results" / "plots" / "Genetic_algorithm"
...
# Convention: lower better for fitness & gini; higher better for top1/top3.
METRICS = [
...
#  Tee logger 
class Tee:
    """Tee/stdout: write to console and to Markdown file."""
    ...
    def __enter__(self):
     ...
     self._file.write(f"# Genetic Algorithm Analysis Output\n\n**Timestamp:** {ts}\n\n**Source CSV:** `{CSV_PATH}`\n\n")
     sys.stdout = self
     return self
...
def clean_and_prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _label(tag: str) -> str:
        s = str(tag or "")
        if "greedy" in s: return "GA Greedy"
        if "random" in s: return "GA Random"
        return s or "GA"
    df["tag_label"] = df["tag"].map(_label) if "tag" in df.columns else "GA"
    if "timestamp" in df.columns:
        df = df.sort_values("timestamp").reset_index(drop=True)
    return df

#  Original stability (bullets, not wide table) 
...
def plot_heatmap(mat: pd.DataFrame, title: str, fname: str, vmin=-1, vmax=1):
    ...
    ax = sns.heatmap(
    mat, annot=True, fmt=".2f", annot_kws={"size": 9}, cmap="coolwarm_r",
    vmin=vmin, vmax=vmax, center=0, cbar_kws={"shrink": 0.85}
)
...
def run_correlations_and_plots(df: pd.DataFrame):
    ...
    plot_heatmap(
    spear,
    "Genetic Algorithm:\nSpearman Correlations (Overall, 30 Runs)",
    "ga_corr_heatmap_overall.png"
)

    # Only 3 key scatters for lean output
    pairs = [
    ("fitness_score", "total_violations", "Genetic Algorithm: Fitness Score vs Total Violations", "scatter_fitness_vs_violations.png"),
    ("fitness_score", "runtime_sec", "Genetic Algorithm: Fitness Score vs Runtime (seconds)", "scatter_fitness_vs_runtime.png"),
    ("top1_pct", "gini_satisfaction", "Genetic Algorithm: Top-1 Preference Match Rate vs Gini Index", "scatter_top1_vs_gini.png"),
]
...
def print_by_tag_compact(df: pd.DataFrame, metrics: List[str]):
    ...
    print_header("GA — Descriptive Summary by Tag")
...
#  Feasibility (integer-aligned histogram) 
def feasibility_rate(df: pd.DataFrame):
    ...
    ax.set_title("Genetic Algorithm: Distribution of Total Violations")
    ax.set_xlabel("Total Violations per run"); ax.set_ylabel("Count of runs")
    ax.set_xlim(x_min - 0.5, x_max + 0.5); ax.set_xticks(np.arange(x_min, x_max + 1, 1))
    sns.despine(); plt.tight_layout()
    out = PLOTS_DIR / "ga_hist_total_violations.png"
    plt.savefig(out, dpi=300)
    if SHOW_IN_SPYDER: plt.show(block=False)
    print(f"[SAVE] {out.name}")

#  Optional: GA correlations 
def ga_param_correlations(df: pd.DataFrame):
    """
    Spearman correlations between GA hyperparameters and outcome metrics.
    Skips params with no variance; coerces to numeric; prints clean rows.
    """
    candidates = ["population_size", "pop_size", "cx_prob", "crossover_rate",
                  "mut_prob", "mutation_rate", "elitism", "selection"]
    params = [c for c in candidates if c in df.columns]
    metrics = [c for c, _ in METRICS if c in df.columns]
    ...
    for p in params:
        ...
        n_param = int(x.notna().sum())
        uniq_param = x.nunique(dropna=True)
        if uniq_param < 3:
            rows.append({"param": p, "metric": "—", "rho": "constant", "p": "—", "n": n_param})
            continue
        for m in metrics:
            y = pd.to_numeric(df[m], errors="coerce")
            mask = x.notna() & y.notna()
            n = int(mask.sum())
            if n < 5 or y[mask].nunique(dropna=True) < 3:
                continue
            rho, pval = stats.spearmanr(x[mask], y[mask])
            if np.isnan(rho) or np.isnan(pval):
                continue
            rows.append({"param": p, "metric": m, "rho": float(rho), "p": float(pval), "n": n})

    print_header("GA — Spearman Correlations: Params vs Metrics")
    if rows:
        out = pd.DataFrame(rows)
        const = out[out["rho"].astype(str) == "constant"]
        corr  = out[out["rho"].astype(str) != "constant"].sort_values(["param","metric"])
        table = pd.concat([const, corr], ignore_index=True)
        print(tabulate(table.round(3), headers="keys", tablefmt="github"))
    else:
        print("(No varying GA parameters; correlations skipped.)")

def ga_ops_correlations(df: pd.DataFrame):
    """
    Spearman correlations between GA operational counters and outcome metrics.
    Skips ops with no variance; coerces to numeric; prints clean rows.
    """
    ops = [c for c in ["cx_ops", "mut_ops", "repair_calls", "repair_per_op"] if c in df.columns]
    metrics = [c for c, _ in METRICS if c in df.columns]
    if not ops or not metrics:
        return

    rows = []
    for op in ops:
        x = pd.to_numeric(df[op], errors="coerce")
        if x.nunique(dropna=True) < 3:
            rows.append({"op": op, "metric": "—", "rho": "constant", "p": "—", "n": int(x.notna().sum())})
            continue
        for m in metrics:
            y = pd.to_numeric(df[m], errors="coerce")
            mask = x.notna() & y.notna()
            ...
            if np.isnan(rho) or np.isnan(pval):
                continue
            rows.append({"op": op, "metric": m, "rho": float(rho), "p": float(pval), "n": n})

    print_header("GA — Spearman Correlations: Ops vs Metrics")
    if rows:
        out = pd.DataFrame(rows)
        const = out[out["rho"].astype(str) == "constant"]
        corr  = out[out["rho"].astype(str) != "constant"].sort_values(["op","metric"])
        table = pd.concat([const, corr], ignore_index=True)
        print(tabulate(table.round(3), headers="keys", tablefmt="github"))
    else:
        print("(No varying operational counters or insufficient data; correlations skipped.)")
...
def save_enhanced_tables(enh: pd.DataFrame):
    ...
    # Excel-friendly copy (ASCII headers, no special symbols)
    enh_excel = enh.copy()
    tick_map = {"Yes": "Yes", "No": "No", "True": "Yes", "False": "No"}
...
def main():
    if not CSV_PATH.exists():
        raise FileNotFoundError(f"GA summary not found: {CSV_PATH}")
    ...
    with Tee(SUMMARY_DIR / "ga_analysis_output.md"):
        ...
        # Optional: GA correlations
        ga_param_correlations(df)  # hyperparams vs metrics (skips constants)
        ga_ops_correlations(df)    # operational counters vs metrics
