# spa_analysis/scripts/analyze_selector_hc_stats.py

"""
Statistical analysis for Selector Hill Climbing batch results.

- Enhanced stability tests (MW + BH/FDR, Cliff’s δ, bootstrap CIs, TOST)
- Concise original tests
- Descriptives (overall + by tag)
- Feasibility rate + histogram
- Spearman heatmap + 3 key scatterplots
- Top-5 tables
- Tee console to Markdown
- Enhanced results saved as CSV + XLSX

Notes:
- Only the 30 most recent runs are analyzed (by timestamp).
"""
...
RUNS_DIR  = ROOT / "results" / "selector_hill_climbing_runs"
CSV_PATH  = RUNS_DIR / "selectorhc_summary_log.csv"
PLOTS_DIR = ROOT / "results" / "plots" / "selector_hill_climbing"
...
# Metrics
METRICS = [
    ("fitness_score", "Fitness Score (lower=better)"),
...
PRACTICAL_DELTA = {
    ...
    "fitness_score": ("rel", 0.01),
    "total_violations": 1.0,
    "runtime_sec": ("rel", 0.05),
}
...
class Tee:
    ...
    def __enter__(self):
        ...
        self._file.write(f"# Selector Hill Climbing — Analysis Output\n\n**Timestamp:** {ts}\n\n**Source CSV:** `{CSV_PATH}`\n\n")
        sys.stdout = self
        return self
    def write(self, data):
...
def clean_and_prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _label(tag: str) -> str:
        s = str(tag or "")
        if "greedy" in s.lower(): return "Selector Hill Climbing Greedy"
        if "random" in s.lower(): return "Selector Hill Climbing Random"
        return "Selector Hill Climbing"
    df["tag_label"] = df["tag"].map(_label) if "tag" in df.columns else "Selector Hill Climbing"

    # Keep ONLY 30 most recent runs
...
def test_block(dfA, dfB, metric_key, metric_label, nameA, nameB):
    ...
    return {
        "Metric": metric_label,
        "Group A": nameA, "nA": len(a),
        "Group B": nameB, "nB": len(b),
        "MW p": mw_p, "Welch t": t_stat, "t p": t_p,
        "Cohen d": d, "Rank-biserial r": r
    }
...
def print_stability_compact(df: pd.DataFrame, title: str):
    ...
    for _, r in df.iterrows():
        ...
        print(f"- {metric}: {a} (n={nA}) vs {b} (n={nB}) → MW p={mwp:.3f}, t={tval:.3f} (p={tp:.3f}), d={d:.3f}, r={rb:.3f}")
...
def practical_threshold(metric_key: str, medA: float, medB: float) -> Tuple[float, bool, str]:
    ...
    lower_better = {"fitness_score","avg_rank","gini_satisfaction","total_violations","runtime_sec"}
    raw_change = (medA - medB) if metric_key in lower_better else (medB - medA)
    ...
    for split in splits:
        for key, label in METRICS:
            ...
            rows.append({
                ...
                "Practical Effect": eff_val, "Practically Meaningful?": "Yes" if is_prac else "No",
                "Favored Group": favored
            })
    if not rows: return pd.DataFrame()
    df = pd.DataFrame(rows)
    df["q (BH)"] = benjamini_hochberg(df["MW p"].tolist())
    df["Significant? (q<0.05)"] = df["q (BH)"].apply(lambda q: "Yes" if q < 0.05 else "No")
    return df
...
def plot_heatmap(mat: pd.DataFrame, title: str, fname: str, vmin=-1, vmax=1):
    # Map raw metric names to human-readable labels on axes
    mat_disp = mat.copy()
    mat_disp.index = [LABELS.get(c, c) for c in mat_disp.index]
    mat_disp.columns = [LABELS.get(c, c) for c in mat_disp.columns]

    plt.figure(figsize=(7.6, 6.0))
    ax = sns.heatmap(
        mat_disp, annot=True, fmt=".2f",
        cmap="RdBu", vmin=vmin, vmax=vmax, center=0,           # positive=blue, negative=red
        annot_kws={"fontsize": 11},                            # a bit smaller numbers
        cbar_kws={"shrink": 0.85}
    )
    # make colorbar ticks a bit smaller too
...
def reg_scatter(df, xcol, ycol, title, fname):
    if xcol not in df.columns or ycol not in df.columns: return
...
def run_correlations_and_plots(df: pd.DataFrame):
    ...
    plot_heatmap(
        spear,
        f"Selector Hill Climbing — Spearman Correlations (Overall, {len(df)} runs)",
        "selectorhc_corr_heatmap_overall.png"
    )

    # Titles with full wording; break Top-1 onto two lines for readability
    pairs = [
        ("fitness_score", "total_violations",
         "Selector Hill Climbing — Fitness vs Total Violations",
         "selectorhc_scatter_fitness_vs_violations.png"),
        ("fitness_score", "runtime_sec",
         "Selector Hill Climbing — Fitness vs Runtime (seconds)",
         "selectorhc_scatter_fitness_vs_runtime.png"),
        ("top1_pct", "gini_satisfaction",
         "Selector Hill Climbing — Top 1 Preference Match Rate\nvs Fairness (Gini)",
         "selectorhc_scatter_top1_vs_gini.png"),
    ]
...
def print_median_iqr(df, cols):
    ...
    for c in cols:
        ...
        out.append({"Metric": LABELS.get(c, c), "Median": med, "IQR": f"[{q1:.3f}–{q3:.3f}]"})
    print_header("Selector Hill Climbing — Median (IQR)")
...
def print_by_tag_compact(df: pd.DataFrame, metrics: List[str]):
    ...
    print_header("Selector Hill Climbing — Descriptive Summary by Tag")
...
def print_top5_tables(df):
    def top5(by, asc=True, title=""):
        ...
        tbl = df.sort_values(by, ascending=asc).loc[:, cols].head(5).rename(columns=LABELS).round(3)
        print_header(title); print(tabulate(tbl, headers="keys", tablefmt="github"))
...
def feasibility_rate(df: pd.DataFrame):
    ...
    x = pd.to_numeric(df["total_violations"], errors="coerce").dropna().astype(int)
    feasible = int((x == 0).sum()); total = int(len(x))
    rate = (feasible / total * 100) if total else 0.0

    print_header("Feasibility")
    print(f"Feasible runs (zero violations): {feasible}/{total} ({rate:.1f}%)")

    x_min = int(x.min()); x_max = int(x.max())
    plt.figure(figsize=(7.2, 5.2))
    try:
        ax = sns.histplot(x, discrete=True, shrink=0.9, color=COLOR, edgecolor=None, linewidth=0, stat="count")
    except TypeError:
        bins = np.arange(x_min - 0.5, x_max + 1.5, 1)
        ax = sns.histplot(x, bins=bins, color=COLOR, edgecolor=None, linewidth=0, stat="count")
    ax.set_title("Distribution of Total Violations — Selector Hill Climbing")
    ax.set_xlabel("Total Violations per run")
    ax.set_ylabel("Count of runs")
    ax.set_xlim(x_min - 0.5, x_max + 0.5)
    ax.set_xticks(np.arange(x_min, x_max + 1, 1))
    sns.despine()
    plt.tight_layout()
    out = PLOTS_DIR / "selectorhc_hist_total_violations.png"
    plt.savefig(out, dpi=300)
    if SHOW_IN_SPYDER: plt.show(block=False)
    print(f"[SAVE] {out.name}")

#  operator usage 
def _parse_operator_map(cell) -> Dict[str, float]:
    ...
    return {k: float(v) for k, v in pairs} if pairs else {}

def _normalize_op_label(op_raw: str) -> str:
    op = re.sub(r"^(op|operator)_", "", op_raw, flags=re.IGNORECASE)
    op = re.sub(r"__+", "_", op)
    op = re.sub(r"_?hh$", "", op, flags=re.IGNORECASE)
    return op

def selectorhc_operator_usage_section(df: pd.DataFrame):
    usage_series = None
    if "operator_seq" in df.columns:
        counter = Counter()
        for cell in df["operator_seq"].dropna():
            parsed = _parse_operator_map(cell)
            parsed_norm = {_normalize_op_label(k): float(v) for k, v in parsed.items()}
            counter.update(parsed_norm)
        if counter:
            usage_series = pd.Series(counter, name="used_count").sort_values(ascending=False)
    if usage_series is None:
        usage_cols = [c for c in df.columns if re.match(r"^(op|operator)_", c, flags=re.IGNORECASE)]
        if usage_cols:
            sums = {}
            for c in usage_cols:
                vals = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
                if vals.sum() > 0:
                    op = _normalize_op_label(c)
                    sums[op] = float(vals.sum())
            if sums:
                usage_series = pd.Series(sums, name="used_count").sort_values(ascending=False)

    accept_cols = [c for c in df.columns if re.match(r"^(accepted|accept)_", c, flags=re.IGNORECASE)]
    acceptance_series = None
    if accept_cols:
        acc = {}
        for c in accept_cols:
            op = re.sub(r"^(accepted|accept)_", "", c, flags=re.IGNORECASE)
            op = _normalize_op_label(op)
            vals = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
            if vals.sum() > 0:
                acc[op] = float(vals.sum())
        if acc:
            acceptance_series = pd.Series(acc, name="accepted_count")

    if usage_series is None or usage_series.empty:
        print_header("Selector Hill Climbing — Operator Usage (sum across runs)")
        print("(no operator usage data found in 'operator_seq' or op_* columns)")
        return

    print_header("Selector Hill Climbing — Operator Usage (sum across runs)")
    print(tabulate(usage_series.reset_index().rename(columns={"index": "operator"}), headers="keys", tablefmt="github", floatfmt=".3f"))

    plt.figure(figsize=(8.5, 5.0))
    ax = sns.barplot(x=usage_series.index, y=usage_series.values, color=COLOR, edgecolor=None, linewidth=0)
    ax.set_title("Selector Hill Climbing Operator Usage (sum across runs)")
    ax.set_xlabel("Operator"); ax.set_ylabel("Used count")
    plt.xticks(rotation=30, ha="right")
    sns.despine(); plt.tight_layout()
    out = PLOTS_DIR / "selectorhc_operator_usage.png"
    plt.savefig(out, dpi=300)
    if SHOW_IN_SPYDER: plt.show(block=False)
    print(f"[SAVE] {out.name}")

    if acceptance_series is not None and not acceptance_series.empty:
        df_rate = pd.concat([usage_series, acceptance_series], axis=1).fillna(0.0)
        df_rate["accept_rate"] = (df_rate["accepted_count"] / df_rate["used_count"]).replace([np.inf, -np.inf], np.nan)

        print_header("Selector Hill Climbing — Operator Acceptance")
        print(tabulate(df_rate.reset_index().rename(columns={"index": "operator"}).round(3), headers="keys", tablefmt="github"))

        plt.figure(figsize=(8.5, 5.0))
        ax = sns.barplot(x=df_rate.index, y=df_rate["accept_rate"].values, color="#2ca02c", edgecolor=None, linewidth=0)
        ax.set_title("Selector Hill Climbing Operator Acceptance Rate")
        ax.set_xlabel("Operator"); ax.set_ylabel("Acceptance rate")
        plt.xticks(rotation=30, ha="right")
        sns.despine(); plt.tight_layout()
        out = PLOTS_DIR / "selectorhc_operator_acceptance_rate.png"
        plt.savefig(out, dpi=300)
        if SHOW_IN_SPYDER: plt.show(block=False)
        print(f"[SAVE] {out.name}")
...
def save_enhanced_tables(enh: pd.DataFrame):
    ...
    enh_excel = enh.copy()
    tick_map = {"Yes": "Yes", "No": "No", "Yes": "True", "False": "No"}
...
def main():
    if not CSV_PATH.exists():
        raise FileNotFoundError(f"Selector Hill Climbing summary not found: {CSV_PATH}")
    ...
    with Tee(SUMMARY_DIR / "selectorhc_analysis_output.md"):
        print("\n# Selector Hill Climbing — Analysis Summary")

        # Descriptives
        desc_cols = [k for k,_ in METRICS if k in df.columns]
        desc = df[desc_cols].describe().T.rename(index=LABELS)
        ...
        # Operator usage/acceptance
        selectorhc_operator_usage_section(df)

        # Feasibility
        feasibility_rate(df)

        # Best/Worst by fitness
        if "fitness_score" in df.columns and len(df) > 0:
            ...
            print(tabulate(
                [{
                    ...
                    "Fitness Score":best["fitness_score"], "Gini Index":best.get("gini_satisfaction",np.nan),
                    "Total Violations":best.get("total_violations",np.nan), "Runtime (seconds)":best.get("runtime_sec",np.nan)
                },
                {
                    "Type":"Worst", "run_id":worst.get("run_id",""), "tag":worst.get("tag_label",""),
                    "Fitness Score":worst["fitness_score"], "Gini Index":worst.get("gini_satisfaction",np.nan),
                    "Total Violations":worst.get("total_violations",np.nan), "Runtime (seconds)":worst.get("runtime_sec",np.nan)
                }],
